...
public interface ContactRepository extends CrudRepository<Contact, Integer> {
    ...
    @Query("SELECT c FROM Contact c WHERE c.person.id = :person_id")
    List<Contact> findByID(@Param("person_id") Integer person_id);

    @Query("SELECT c FROM Contact c WHERE (c.person.id) = (:person_id) AND LOWER(contact_type) = \'work\'")
    List<Contact> findByIDandWork(@Param("person_id") Integer person_id);

    @Query("SELECT c FROM Contact c WHERE (c.person.id) = (:person_id) AND LOWER(contact_type) = \'home\'")
    List<Contact> findByIDandHome(@Param("person_id") Integer person_id);

    @Query("SELECT c FROM Contact c WHERE (c.person.id) = (:person_id) AND LOWER(contact_detail) = LOWER(:contact_detail)")
