...
public interface ContactRepository extends CrudRepository<Contact, Integer> {
    ...
    /*
     * @Query("SELECT c FROM Contact c WHERE (c.person.person_id) = (:person_id) AND LOWER(c.person.contact_type) = \'work\'"
     * ) List<Contact> findByIDandWork(@Param("person_id") Integer person_id);
     * 
     * @Query("SELECT c FROM Contact c WHERE (c.person.person_id) = (:person_id) AND LOWER(c.person.contact_type) = \'home\'"
     * ) List<Contact> findByIDandHome(@Param("person_id") Integer person_id);
     */
    @Query("SELECT c FROM Contact c WHERE (c.person.person_id) = (:person_id) AND LOWER(c.person.contact_type) = LOWER(:contact_type) and LOWER(c.person.contact_detail) and LOWER(:contact_detail)")
